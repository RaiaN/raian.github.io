...
Can the above code be used? Sure. It is a simple solution, which works well for medium-resolution images. However there are pros too. One example is when a set of high resolution images (4k or even 8k) needs to be loaded at runtime without overloading render thread.
...
Overall this approach works well with the high resolution textures and also gives a great flexibility in terms of what and when the texture related objects should be created. For example, generating texture mips on the background thread and then uploading them to GPU via *RHICopySharedMips* is one of such examples. This approach is used in author's Unreal plugin called [**RuntimeImageLoader**](https://github.com/RaiaN/ue4_runtimeimageloader), which is available for free on Github. 
